...
plot(x, fx(x), type="l", col=c10) # genera grafica
```
...
grafica(z,c10)  # evalua la funcion en los datos z y colo rojo
```


```{r}
w=round(prop.table(z)*100,2)  # genera datos y constuye tabla en porcentaje
grafica(w,c60)  # evalua la funcion con w y color azul
```
...
# Descargar datos de desempleo mensual de Colombia (serie COLLRHUTTTTSTSAM)
getSymbols("COLLRHUTTTTSTSAM", src = "FRED")

# Convertir a serie de tiempo y revisar últimos datos
desempleo_col <- COLLRHUTTTTSTSAM
